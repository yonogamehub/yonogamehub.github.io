...
function renderFirebaseGames(games = getVisibleGames()) {
    ...
    renderTopApps(getVisibleGames().slice(0, 3));
    animateCards();
}

window.renderFirebaseGames = renderFirebaseGames;
window.setFirebaseGames = function(games) {
    allFirebaseGames = Array.isArray(games) ? games.slice() : [];
    renderFirebaseGames();
};
...
function renderTopApps(games = allFirebaseGames) {
    ...
    const markedTop = allFirebaseGames
    ...
    // If Manage Games has no Top 3 selected yet, show the first 3 games by order.
    const topGames = (markedTop.length ? markedTop : allFirebaseGames.slice().sort((a,b) => getOrder(a)-getOrder(b)))
        .slice(0, 3);
    ...
    container.innerHTML = topGames.map((game, index) => {
        ...
        <div class="top-app-card">
            ...
            <img src="${image}" alt="${name}" class="top-app-image"
                 onerror="this.onerror=null;this.src='images/logo.png';">
            <h3>${name}</h3>
            <p>🎁 <b>Sign Up Bonus ₹${bonus}</b></p>
            <small>🏦 <b>Min Withdraw ₹${withdraw}</b></small>
            ${download}
        </div>`;
    }).join("");
}
